...
begin
  ListView_usuario.Items.Clear;  //Limpa informações do ListView
  DM.FDConnexao.Close;   //Fecha conexão com Banco
  DM.FDConnexao.Open();  //Abre conexão com Banco
  DM.QueryUsuarios.Open; //Executa a Query de Consultar Usuários

  with ListView_usuario.Items.Add do  // Adiciona informação no List View
  begin
    TListItemText(Objects.FindDrawable('Linha')).Text := 'Nome Completo | Usuário';
  end;

  while not DM.QueryUsuarios.Eof do  // Enquanto tiver informação no Select ira seguir conforme abaixo
  begin
    with ListView_usuario.Items.Add do //Ira adicionar no ListView as seguintes informações
    begin
        nome :=DM.QueryUsuarios.FieldByName('nome').AsString;  // Nome completo do usuário
        usuario :=DM.QueryUsuarios.FieldByName('usuario').AsString;   // Nome do usuário
        TListItemText(Objects.FindDrawable('Linha')).Text := nome + ' | ' + usuario;  // Escreve na linha concatenando nome e usuário
        DM.QueryUsuarios.Next; //Passa para a próxima linha do resultado do Select
    end;
...
begin
  ShowMessage('Disponível na próxima versão :)');
end;
